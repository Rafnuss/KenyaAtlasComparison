...
% sp(sp.common_name=="Levant Sparrowhawk",:) = [];
...
% As in G_sabap.m: this file's SEQ keys predate the merged_SEQ folds. Here the
% fold that matters is an identification decision, not a taxonomic one, so
% fold_SEQ drops European Herring Gull (SEQ 299) rather than adding its trend
% to Lesser Black-backed Gull (SEQ 298), which burns2021 already reports.
burn.SEQ = fold_SEQ(burn.SEQ);
...
% text(spj.diff(id), spj.annualRateOfChange(id)+.001,spj.common_name_sp(id),"HorizontalAlignment","center")
...
%%
id2 = id & contains(spj.common_name_sp,"Warbler");
figure; hold on;
scatter(spj.diff(id2), spj.annualRateOfChange(id2),(spj.new(id2) + spj.old(id2)),'ok','filled')
text(spj.diff(id2), spj.annualRateOfChange(id2)+.001,spj.common_name_sp(id2),"HorizontalAlignment","center")
